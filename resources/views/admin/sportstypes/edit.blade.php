...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.sportstypes.update", [$sportstype->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="sport_name">{{ trans('cruds.sportstype.fields.sport_name') }}</label>
                <input class="form-control {{ $errors->has('sport_name') ? 'is-invalid' : '' }}" type="text" name="sport_name" id="sport_name" value="{{ old('sport_name', $sportstype->sport_name) }}" required>
