...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-Sportstype">
                <thead>
                    <tr>
                        <th width="10">
                ...
                <tbody>
                    @foreach($sportstypes as $key => $sportstype)
                        <tr data-entry-id="{{ $sportstype->id }}">
                            ...
                            <td>
                                @can('sportstype_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.sportstypes.show', $sportstype->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('sportstype_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.sportstypes.edit', $sportstype->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan
...
@endcan
  ...
  let table = $('.datatable-Sportstype:not(.ajaxTable)').DataTable({ buttons: dtButtons })
