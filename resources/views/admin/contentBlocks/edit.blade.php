...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.contentBlock.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.content-blocks.update", [$contentBlock->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="name_of_the_block">{{ trans('cruds.contentBlock.fields.name_of_the_block') }}</label>
                <input class="form-control {{ $errors->has('name_of_the_block') ? 'is-invalid' : '' }}" type="text" name="name_of_the_block" id="name_of_the_block" value="{{ old('name_of_the_block', $contentBlock->name_of_the_block) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('heading') ? 'is-invalid' : '' }}" type="text" name="heading" id="heading" value="{{ old('heading', $contentBlock->heading) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('body') ? 'is-invalid' : '' }}" name="body" id="body">{!! old('body', $contentBlock->body) !!}</textarea>
                @if($errors->has('body'))
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\ContentBlock::STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $contentBlock->status) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('source_page') ? 'is-invalid' : '' }}" name="source_page_id" id="source_page_id">
                    @foreach($source_pages as $id => $entry)
                        <option value="{{ $id }}" {{ (old('source_page_id') ? old('source_page_id') : $contentBlock->source_page->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
