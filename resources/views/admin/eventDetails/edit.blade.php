...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.eventDetail.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.event-details.update", [$eventDetail->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="event_title">{{ trans('cruds.eventDetail.fields.event_title') }}</label>
                <input class="form-control {{ $errors->has('event_title') ? 'is-invalid' : '' }}" type="text" name="event_title" id="event_title" value="{{ old('event_title', $eventDetail->event_title) }}" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('event_details') ? 'is-invalid' : '' }}" name="event_details" id="event_details">{!! old('event_details', $eventDetail->event_details) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('event_date') ? 'is-invalid' : '' }}" type="text" name="event_date" id="event_date" value="{{ old('event_date', $eventDetail->event_date) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('gallery') ? 'is-invalid' : '' }}" name="gallery_id" id="gallery_id">
                    @foreach($galleries as $id => $entry)
                        <option value="{{ $id }}" {{ (old('gallery_id') ? old('gallery_id') : $eventDetail->gallery->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
