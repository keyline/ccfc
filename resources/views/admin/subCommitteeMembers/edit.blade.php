...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.sub-committee-members.update", [$subCommitteeMember->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('comittee_name') ? 'is-invalid' : '' }}" name="comittee_name_id" id="comittee_name_id" required>
                    @foreach($comittee_names as $id => $entry)
                        <option value="{{ $id }}" {{ (old('comittee_name_id') ? old('comittee_name_id') : $subCommitteeMember->comittee_name->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('member') ? 'is-invalid' : '' }}" name="member_id" id="member_id">
                    @foreach($members as $id => $entry)
                        <option value="{{ $id }}" {{ (old('member_id') ? old('member_id') : $subCommitteeMember->member->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('head_of_the_committee') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="head_of_the_committee" id="head_of_the_committee" value="1" {{ $subCommitteeMember->head_of_the_committee || old('head_of_the_committee', 0) === 1 ? 'checked' : '' }}>
