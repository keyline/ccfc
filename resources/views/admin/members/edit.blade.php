    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.members.update", [$member->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('select_member') ? 'is-invalid' : '' }}" name="select_member_id" id="select_member_id" required>
                    @foreach($select_members as $id => $entry)
                        <option value="{{ $id }}" {{ (old('select_member_id') ? old('select_member_id') : $member->select_member->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('select_title') ? 'is-invalid' : '' }}" name="select_title_id" id="select_title_id" required>
                    @foreach($select_titles as $id => $entry)
                        <option value="{{ $id }}" {{ (old('select_title_id') ? old('select_title_id') : $member->select_title->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
