...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.amenitiesService.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.amenities-services.update", [$amenitiesService->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="amenity_name">{{ trans('cruds.amenitiesService.fields.amenity_name') }}</label>
                <input class="form-control {{ $errors->has('amenity_name') ? 'is-invalid' : '' }}" type="text" name="amenity_name" id="amenity_name" value="{{ old('amenity_name', $amenitiesService->amenity_name) }}" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('amenity_details') ? 'is-invalid' : '' }}" name="amenity_details" id="amenity_details">{!! old('amenity_details', $amenitiesService->amenity_details) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('image_gallery') ? 'is-invalid' : '' }}" name="image_gallery_id" id="image_gallery_id">
                    @foreach($image_galleries as $id => $entry)
                        <option value="{{ $id }}" {{ (old('image_gallery_id') ? old('image_gallery_id') : $amenitiesService->image_gallery->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
